...
from architectures.layers import ModLogReg
...
def bow_linear(epochs:int, scores:bool=False)->None:
   ...
   if scores:
      model = ModLogReg(train_data[0].shape[1], train_data[1].shape[1], return_scores=True)
   else:
      model = ModLogReg(train_data[0].shape[1], train_data[1].shape[1])
